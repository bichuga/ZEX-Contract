...
// ZexToken with Governance.
contract ZexSwap is BEP20('ZexSwap', 'Zex') {
...
// ZexMaster is the master of Egg. He can make Egg and he is a fair guy.
//
...
contract ZexMaster is Ownable {
    ...
    struct PoolInfo {
        ...
        uint256 accZexPerShare;   // Accumulated ZEX per share, times 1e12. See below.
    ...
    // Zex tokens created per block.
    uint256 public ZexPerBlock;
    ...
    constructor(
        ...
        address _devaddr,
        address _feeAddress,
        uint256 _zexPerBlock,
        uint256 _startBlock
    ) public {
        Zex = _zex;
        devaddr = _devaddr;
        feeAddress = _feeAddress;
        ZexPerBlock = _zexPerBlock;
    ...
    // Update the given pool's ZEX allocation point and deposit fee. Can only be called by the owner.
    ...
    function pendingZex(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            uint256 ZexReward = multiplier.mul(ZexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
            accZexPerShare = accZexPerShare.add(ZexReward.mul(1e12).div(lpSupply));
        }
        return user.amount.mul(accZexPerShare).div(1e12).sub(user.rewardDebt);
    }
    ...
    function updatePool(uint256 _pid) public {
        ...
        uint256 multiplier = getMultiplier(pool.lastRewardBlock, block.number);
        uint256 ZexReward = multiplier.mul(ZexPerBlock).mul(pool.allocPoint).div(totalAllocPoint);
        Zex.mint(devaddr, ZexReward.div(10));
        Zex.mint(address(this), ZexReward);
        pool.accZexPerShare = pool.accZexPerShare.add(ZexReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }
    ...
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        user.rewardDebt = user.amount.mul(pool.accZexPerShare).div(1e12);
